...
class Follower:
    def __init__(self):
        ...
        image = rospy.Subscriber('/unity_image/compressed_d6', CompressedImage, self.image_callback)
    ...
    def callback(self, data):
        ...
        if d06_posZ < 3 and dist_56 > 15:            
            calc_odom = (Vector3(-0.1*(d06_velX-d05_posX),-0.1*(d06_posY-d05_posY),-0.05*(d06_posZ-3)))
            control = [-0.1*(d06_velX-d05_posX),-0.1*(d06_posY-d05_posY),-0.05*(d06_posZ-3)]
            print("ATR APPEAR!!")

        else:
            if d06_posZ >= 3 and curr_time > 10:
            ...
            new_velX = velX - repul
            control = [new_velX,velY,-2.0*(d06_posZ-5)]
            calc_odom = (Vector3(new_velX,velY,-2.0*(d06_posZ-5)))
