...
class Follower:
    def __init__(self):
        ...
        image = rospy.Subscriber('/unity_image/compressed_d7', CompressedImage, self.image_callback)
    ...
    def callback(self, data):
        ...
        if d07_posZ < 3 and dist_87 > 15:            
            calc_odom = (Vector3(-0.1*(d07_velX-d08_posX),-0.1*(d07_posY-d08_posY),-0.05*(d07_posZ-3)))
            control = [-0.1*(d07_velX-d08_posX),-0.1*(d07_posY-d08_posY),-0.05*(d07_posZ-3)]
            print("ATR APPEAR!!")
        else:
            ...
            new_velX = velX - repul
            control = [new_velX,velY,-2.0*(d07_posZ-5)]
            calc_odom = (Vector3(new_velX,velY,-2.0*(d07_posZ-5)))
