...
def calc_attractive_potential(x, y, tx, ty):
    return 1*(tx-x), 1*(ty-y)

def sigmoid(x):
    return 30/(1+math.exp(-x))-30/(1+math.exp(-3.0))

def calc_repulsive_potential(x1, y1, x2, y2):
    dist = math.sqrt((x1-x2)**2 + (y1-y2)**2)
    if(dist<=3):
	force_x = sigmoid(math.sqrt((x1-x2)**2))
	force_y = sigmoid(math.sqrt((y1-y2)**2))
        if(x2>x1):
	    force_x = -force_x
        if(y2>y1):
	    force_y = -force_y
...
def callback(data):
    ...
    d2_orienX = data.pose[4].orientation.x
    d2_orienY = data.pose[4].orientation.y
    d2_orienZ = data.pose[4].orientation.z
    d2_orienW = data.pose[4].orientation.w
    (d2_roll, d2_pitch, d2_yaw) = tf.transformations.euler_from_quaternion([d2_orienX, d2_orienY, d2_orienZ,d2_orienW])
    d2_velLin = data.twist[4].linear
    d2_velAngular = data.twist[4].angular
    d2_posX = data.pose[4].position.x
    d2_posY = data.pose[4].position.y
    d2_posZ = data.pose[4].position.z
    ...
    d1_orienX = data.pose[3].orientation.x
    d1_orienY = data.pose[3].orientation.y
    d1_orienZ = data.pose[3].orientation.z
    d1_orienW = data.pose[3].orientation.w
    (d1_roll, d1_pitch, d1_yaw) = tf.transformations.euler_from_quaternion([d1_orienX, d1_orienY, d1_orienZ,d1_orienW])
    d1_velLin = data.twist[3].linear
    d1_velAngular = data.twist[3].angular
    d1_posX = data.pose[3].position.x
    d1_posY = data.pose[3].position.y
    d1_posZ = data.pose[3].position.z
    ...
    repuls_x_d2, repuls_y_d2 = calc_repulsive_potential(d2_posX, d2_posY, d1_posX, d1_posY)
