...
def callback(data):
    ...
    sin = math.sin(theta)
    cos = math.cos(theta)
    new_velx = t_velLin.x + 3*(t_posX-d_posX)
    new_vely = t_velLin.y + 3*(t_posY-d_posY)
    calc_odom = [new_velx, new_vely,0.01,0,0,0]
    print(new_velx)
    print(new_vely)
    pub.publish(Twist(Vector3(new_velx, new_vely,0.001),Vector3(0,0,0)))
    rospy.Rate(10).sleep
