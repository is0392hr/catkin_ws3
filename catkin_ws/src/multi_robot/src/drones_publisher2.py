...
def callback(data):
    ...
    distance_x = math.fabs(d1_posX-d2_posX)
    distance_y = math.fabs(d1_posY-d2_posY)
    
    dif_dis = math.sqrt(distance_x**2 + distance_y**2)
    print(dif_dis)
    update_x_d1 = 2*(d1_posX-d2_posX)
    update_x_d2 = 2*(d2_posX-d1_posX)
    update_y_d1 = 2*(d1_posY-d2_posY)
    update_y_d2 = 2*(d2_posY-d1_posY)
    new_velx_d1 = t_velLin.x + 3*(t_posX-d1_posX)
    new_vely_d1 = t_velLin.y + 3*(t_posY-d1_posY)
    new_velx_d2 = t_velLin.x + 3*(t_posX-d2_posX)
    new_vely_d2 = t_velLin.y + 3*(t_posY-d2_posY)
    if dif_dis<5:
	    if distance_x<1:
		new_velx_d1 = new_velx_d1 + update_x_d1
		new_velx_d2 = new_velx_d2 + update_x_d2
		print('dangerous')
	    if distance_y<1:
		new_vely_d1 = new_vely_d1 + update_y_d1
		new_vely_d2 = new_vely_d2 + update_y_d2
		print('dangerous')
    if d2_posZ>5:
